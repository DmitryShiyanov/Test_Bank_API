...
class TestBankAccountDeposit:
    ...
    def test_account_deposit_with_valid_amount(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest, amount: int):
        response = api_manager.user_steps.create_account(create_user_request)

        account_deposit = AccountDepositRequest(accountId=response.id, amount=amount)
    ...
    def test_account_deposit_with_invalid_amount(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest, amount: int):
        response = api_manager.user_steps.create_account(create_user_request)

        account_before = Account.get_account_by_id(db_session, response.id)
        ...
        account_after = Account.get_account_by_id(db_session, response.id)
