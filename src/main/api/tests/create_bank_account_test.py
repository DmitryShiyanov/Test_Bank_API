...
class TestCreateBankAccount:
    def test_account_creation(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest, user_account: CreateUserRequest):
        ...
        assert account_from_db.id == user_account.id, "Аккаунт не создан,  ID аккаунта нет в БД"
        assert account_from_db.balance is not None, "Поле баланса для аккаунта отсутствует в БД"

    def test_more_then_two_bank_account_creation(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest, user_with_two_accounts: CreateUserRequest):
